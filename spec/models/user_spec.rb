...
describe User do
  ...
  describe 'accessibility' do
    before :each do
      ...
      @cohort2 = FactoryGirl.create(:cohort, campus: @campus1)
      @cohort3 = FactoryGirl.create(:cohort, campus: @campus2)
      ...
      @user2.administered_cohorts << @cohort3
    end
    ...
    describe '#accessible_users' do
      ...
      describe 'cohort_admin' do
        ...
        it 'returns all users for the cohort even if user is on cohort themselves' do
          ids = [@user3.id]
          expect(@user2.accessible_users.map(&:id).sort).to eq ids
        end
      end

      describe 'user' do
        it 'returns no users' do
          [@user1, @user3, @user4].each do |user|
    ...
    describe '#accessible_cohorts' do
      ...
      describe 'campus_admin' do
        it 'returns all cohorts for the campus' do
      ...
      describe 'cohort_admin' do
        ...
        it 'returns all the administered cohorts and own cohort if user is on cohort themselves' do
          ids = [@cohort2, @cohort3].map(&:id).sort
          expect(@user2.accessible_cohorts.map(&:id).sort).to eq ids
        end
      end
  ...
  it '#accessible_cohorts_by_campus'
